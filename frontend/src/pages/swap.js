...
export default function Swap() {
  ...
  // `reservedCD` keeps track of the TK Dev tokens Reserve balance in the Exchange contract
  ...
  // removeCD is the amount of `TK Dev` tokens that would be sent back to the user based on a certain number of `LP` tokens
  ...
  const [tokenToBeReceivedAfterSwap, settokenToBeReceivedAfterSwap] =
    useState(zero);
  // Keeps track of whether  `Eth` or `TK Dev` token is selected. If `Eth` is selected it means that the user
  // wants to swap some `Eth` for some `TK Dev` tokens and vice versa if `Eth` is not selected
  ...
  const getAmounts = async () => {
    try {
      ...
      // get the amount of `TK Dev` tokens held by the user
      const _cdBalance = await getCDTokensBalance(provider, address);
      // get the amount of `TK Dev` LP tokens held by the user
  ...
  /**
   * swapTokens: Swaps  `swapAmountWei` of Eth/TK Dev tokens with `tokenToBeReceivedAfterSwap` amount of Eth/TK Dev tokens.
   */
  ...
  /**
   * _getAmountOfTokensReceivedFromSwap:  Returns the number of Eth/TK Dev tokens that can be received
   * when the user swaps `_swapAmountWEI` amount of Eth/TK Dev tokens.
   */
  ...
  const _getTokensAfterRemove = async (_removeLPTokens) => {
    try {
      ...
      const _ethBalance = await getEtherBalance(provider, null, true);
      // get the TK dev token reserves from the contract
      const TKDevTokenReserve = await getReserveOfCDTokens(provider);
  ...
  */
      ...
      return (
        <div>
          <div className={"description"}>
            ...
            {utils.formatEther(cdBalance)} TK Dev Tokens
            <br />
            {utils.formatEther(ethBalance)} Ether
            <br />
            {utils.formatEther(lpBalance)} TK Dev LP tokens
          </div>
          <div>
            ...
            {utils.parseEther(reservedCD.toString()).eq(zero) ? (
              <div>
                <input
                  ...
                  className="border-0 px-3 py-3 placeholder-blueGray-300 mb-4 text-black bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                />
                <input
                  type="number"
                  placeholder="Amount of TKDev tokens"
                  onChange={(e) =>
                    setAddCDTokens(
              ...
              <div>
                <input
                  ...
                  className="border-0 px-3 py-3 mb-4 placeholder-blueGray-300 text-black bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                />
                <div className={"inputDiv"}>
                  {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
                  {`You will need ${utils.formatEther(addCDTokens)} TK Dev
                  Tokens`}
                </div>
                <button className={"button1"} onClick={_addLiquidity}>
                  Add
            ...
            <div>
              <input
                ...
                className="border-0 px-3 mb-4 py-3 placeholder-blueGray-300 text-black bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
              />
              <div className={"inputDiv"}>
                {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
                {`You will get ${utils.formatEther(removeCD)} TK
      ...
      return (
        <div>
          ...
          >
            ...
            <option value="TKDevToken">TK Dev Token</option>
          </select>
          <br />
          <div className={"inputDiv"}>
            {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
  ...
  return (
    <div>
      ...
      <div className={"main"}>
        <div>
          <h1 className={"title"}>Welcome to TK Devs Exchange!</h1>
          <div className={"description"}>
            Exchange Ethereum &#60;&#62; TK Dev Tokens
          </div>
          <div>
            <button
              className={"button"}
